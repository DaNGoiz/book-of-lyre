    ...
    public class EditorTools
    {
        ...
        public static List<T> GetPrefabsOfType<T>() where T : Component
        {
            ...
            for (int i = fileNames.Length; i > 0; i--)
            {
                ...
                GameObject go = AssetDatabase.LoadAssetAtPath(fileNames[i - 1], typeof(GameObject)) as GameObject;
                if (go != null)
                {
        ...
        {
            System.Reflection.BindingFlags bfs = BindingFlags.NonPublic | BindingFlags.Static;
        ...
        public static Vector2 RectHandle(Vector2 aPos, Rect aRect, Rect aBounds)
        {
            ...
            DrawRect(new Rect(aBounds.x + aRect.x, aBounds.y + aRect.y, aRect.width, aRect.height));
